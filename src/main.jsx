...
const safeJSON = (key, fallback) => { try { const raw = localStorage.getItem(key); if (!raw) return fallback; const value = JSON.parse(raw); return value == null ? fallback : value; } catch { return fallback; } };
const safeArray = (key, fallback = []) => { const value = safeJSON(key, fallback); return Array.isArray(value) ? value : fallback; };
const makeId = () => (globalThis.crypto?.randomUUID ? globalThis.crypto.randomUUID() : `id-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
function App() {
  ...
  const [watch, setWatch] = useState(() => safeArray("osrsflipit-watch"));
  const [alerts, setAlerts] = useState(() => safeArray("osrsflipit-alerts"));
  const [alertRules, setAlertRules] = useState(() => safeArray("osrsflipit-rules"));
  const [profiles, setProfiles] = useState(() => safeArray("osrsflipit-profiles").filter(p => p && typeof p.name === "string" && Array.isArray(p.conditions)));
  const [conditions, setConditions] = useState(() => safeArray("osrsflipit-filters").filter(c => c && c.id && typeof c.field === "string"));
  ...
  const DEFAULT_COLUMNS = ["buy","sell","margin","roi","volume","limit","potentialProfit","updated"];
  const [columns, setColumns] = useState(() => { const saved = safeArray("osrsflipit-columns", DEFAULT_COLUMNS); const valid = saved.filter(k => columnOptions.some(x => x[0] === k)); return valid.length ? valid : DEFAULT_COLUMNS; });
  ...
  function checkRules(rows) {
    ...
    const changed = alertRules.map(rule => {
      ...
      if (hit && stamp - (rule.lastTriggered || 0) > 10 * 60 * 1000) { fresh.push({ id: makeId(), itemId: item.id, name: item.name, metric: rule.metric, value, target: rule.value, at: stamp }); return { ...rule, lastTriggered: stamp }; }
      return rule;
    });
  ...
  const activeAlerts = alerts.filter(a => a && Number.isFinite(Number(a.at)) && Date.now() - Number(a.at) < 60 * 60 * 1000);
  ...
  function addCondition() { setConditions(c => [...c,{id:makeId(),field:"margin",op:"gte",value:50000}]); }
  ...
  return <div className="app">
    ...
    {selected&&<ItemPanel item={selected} history={history[selected.id]||{}} watch={watch} toggleWatch={toggleWatch} close={()=>setSelected(null)} selectRange={range=>selectItem(selected,range,selectedCenter)} center={selectedCenter} dark={dark}/>} 
...
function ItemPanel({item,history,watch,toggleWatch,close,selectRange,center,dark}) {
...
function MoneyMakers(){
  const [sort,setSort]=useState("gpHour"),[category,setCategory]=useState("All"),[members,setMembers]=useState("All"),[selected,setSelected]=useState(null),[wikiRows,setWikiRows]=useState(()=>safeArray("osrsflipit-money-snapshot")),[source,setSource]=useState("cached");
  useEffect(()=>{
